...
HyperContent构建系统负责将Unity工程资源转换为可被runtime消费的bundles和catalog。作为Owner1，本系统严格遵守Owner0定义的catalog schema（schemaVersion=2，见 RESOURCE_LOADING_SYSTEM_SPEC 与 CatalogSchemaV2.cs）。
...
│  - 验证BuildPlan                                             │
│  - 构建AssetBundles (Unity BuildPipeline)                    │
│  - 生成Catalog (CatalogGenerator)                           │
...
- 生成 catalog（符合 schemaVersion=2，见 RESOURCE_LOADING_SYSTEM_SPEC）
- 生成构建报告（可选）
...
│
├── 打包与生成
│   ├── BundleBuilder.cs                # Bundle构建器（已整合到DefaultBuildExecutor）
...
├── AddressableGroupingTool.cs         # Addressable分组工具实现
└── AddressableGroupingInitializer.cs  # 工具注册初始化器
```
...
- **AddressableGroupingTool** (ID: "addressable")
  - 直接从Addressable Groups读取分组数据
  - 只收集标记为Addressable的资源
  - 使用Addressable的address作为asset key
  - 根据Addressable Group名称分配bundle
  - 生成完整的BuildPlan
  - **位置**: `Assets/Scripts/Editor/AddressableGrouping/AddressableGroupingTool.cs`
  - **特点**: 完全基于Addressable系统，无需HyperContentAsset markers

#### 打包执行器

- **DefaultBuildExecutor** (ID: "default")
  - 验证BuildPlan
  - 使用Unity AssetBundle系统构建bundle
  - 生成符合 schemaVersion=2 的 catalog（.v2.catalog.json）
  - 生成构建报告（可选）
...
- 严格遵守 schemaVersion=2 格式（RESOURCE_LOADING_SYSTEM_SPEC、CatalogSchemaV2）
- 自动生成catalog.json文件
...
6. **执行构建**: 调用打包执行器的`Execute()`方法
   ...
   - 生成 Catalog（符合 schemaVersion=2 的 .v2.catalog.json）
   - 最终验证（验证构建结果）
   - 生成报告（如果启用）
...
2. 准备AssetBundle构建：将BuildPlan转换为Unity AssetBundle构建配置
3. 构建AssetBundles：调用Unity的BuildPipeline.BuildAssetBundles
4. 生成Catalog：使用CatalogGenerator生成catalog.json
5. 生成报告：使用BuildReportGenerator生成构建报告（如果启用）
...
构建系统严格遵守 Owner0 定义的 catalog schema（**schemaVersion=2**）。顶层结构含 `schemaVersion`、`catalogName`、`catalogHash`、`stringTable`、`assetRecords`、`nameAliases`、`bundleRecords`、`guidSortedIndex` 等，详见 [RESOURCE_LOADING_SYSTEM_SPEC](../RESOURCE_LOADING_SYSTEM_SPEC.md) 与 `Runtime/Catalog/CatalogSchemaV2.cs`。

### 接口变更控制

- 任何对 catalog schema 的修改必须先提案给 Owner0
- 收到 Owner0 确认后才能合并
- 当前版本使用 schemaVersion=2，字段与类型以 Owner0 审定为准
...
**AddressableGroupingTool** 是一个专门用于Addressable系统的分组工具，特点：
...
4. **与DefaultGroupingTool的区别**：
   ...
   - AddressableGroupingTool：直接使用Addressable Groups，更简单直接
...
- `BundleBuilder.cs` - 被DefaultBuildExecutor使用
