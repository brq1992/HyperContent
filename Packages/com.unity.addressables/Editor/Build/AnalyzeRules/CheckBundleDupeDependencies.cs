...
namespace UnityEditor.AddressableAssets.Build.AnalyzeRules
{
    ...
    public class CheckBundleDupeDependencies : BundleRuleBase
    {
		...
		//Add by jeff
        public HashSet<string> RefreshAnalysisAndGetMp4s(AddressableAssetSettings settings)
        {
            ClearAnalysis();
            return CheckForDependenciesAndGetMp4s(settings);
        }

		//Add by jeff
        public Dictionary<string, HashSet<string>> RefreshAnalysisAndGetMp4Groups(AddressableAssetSettings settings)
        {
            ClearAnalysis();
            return CheckForDependenciesAndGetMp4Groups(settings);
        }
		...
		//Add by jeff
        private HashSet<string> CheckForDependenciesAndGetMp4s(AddressableAssetSettings settings)
        {
            ...
            HashSet<string> mp4Groups = new HashSet<string>();
            if (AllBundleInputDefs.Count > 0)
            {
                ...
                mp4Groups = GetAllMp4AssetPaths(implicitGuids);
            }
            else
            {
                Debug.LogError("No bundle input defs");
                return new HashSet<string>();
            }
		...
		//Add by jeff
        private Dictionary<string, HashSet<string>> CheckForDependenciesAndGetMp4Groups(AddressableAssetSettings settings)
        {
            if (!BuildUtility.CheckModifiedScenesAndAskToSave())
            {
                Debug.LogError("Cannot run Analyze with unsaved scenes");
            ...
            CalculateInputDefinitions(settings);
            Dictionary<string, HashSet<string>> mp4Groups = new Dictionary<string, HashSet<string>>();
            if (AllBundleInputDefs.Count > 0)
            {
                var context = GetBuildContext(settings);
                ReturnCode exitCode = RefreshBuild(context);
                ...
                var implicitGuids = GetImplicitGuidToFilesMap();
                mp4Groups = GetGroupsForMp4(implicitGuids, context);
            }
            else
            {
                Debug.LogError("No bundle input defs");
		...
		//Add by jeff
        HashSet<string> GetAllMp4AssetPaths(Dictionary<UnityEditor.GUID, List<string>> implicitGuids)
        {
            var mp4Paths = new HashSet<string>();

            foreach (var kvp in implicitGuids)
            {
                string assetPath = AssetDatabase.GUIDToAssetPath(kvp.Key.ToString());

                if (!string.IsNullOrEmpty(assetPath) && assetPath.EndsWith(".mp4", StringComparison.OrdinalIgnoreCase))
                {
                    mp4Paths.Add(assetPath);
                }
            }
		...
		//Add by jeff
        Dictionary<string, HashSet<string>> GetGroupsForMp4(Dictionary<GUID, List<string>> implicitGuids, AddressableAssetsBuildContext aaContext)
        {
            var groupNames = new Dictionary<string, HashSet<string>>();
            ...
            foreach (var kvp in implicitGuids)
            {
                ...
                if(!groupNames.ContainsKey(assetPath))
                {
                    groupNames.Add(assetPath, new HashSet<string>());
                }

                foreach (string file in kvp.Value.Distinct())
                {
                    if (!ExtractData.WriteData.FileToBundle.TryGetValue(file, out var bundleName))
                        continue;

                    if (!aaContext.bundleToAssetGroup.TryGetValue(bundleName, out var groupGuid))
                        continue;

                    var group = aaContext.Settings.FindGroup(g => g != null && g.Guid == groupGuid);
                    if (group != null)
                    {
                        groupNames[assetPath].Add(group.Name);
                    }
                }
            }
